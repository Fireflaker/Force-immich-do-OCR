...
(function() {
    ...
    const eventHandler = (event) => {
        // Allow blur events on specific input elements only
        // CRITICAL: Don't block blur on form elements - they need it for validation
        if (event.type === 'blur' && 
            (event.target instanceof HTMLInputElement ||
             event.target instanceof HTMLTextAreaElement)) {
            return; // Allow blur on form elements
        }
